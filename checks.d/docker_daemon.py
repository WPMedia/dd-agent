...
class DockerDaemon(AgentCheck):
    ...
    def _report_disk_stats(self):
        ...
        old_aliases = {
            'docker.data.used': 'docker.info.data_space_used',
            'docker.data.total': 'docker.info.data_space_total',
            'docker.data.free': 'docker.info.data_space_free',
            'docker.metadata.used': 'docker.info.metadata_space_used',
            'docker.metadata.total': 'docker.info.metadata_space_total',
            'docker.metadata.free': 'docker.info.metadata_space_free'
        }
        ...
        for metric in driver_status:
            ...
            if len(metric) == 2 and 'Space' in metric[0]:
                ...
                mkey = None
                if 'Used' in metric[0]:
                    mkey = 'docker.{0}.used'.format(mtype)
                    stats[mkey] = metric[1]
                elif 'Space Total' in metric[0]:
                    mkey = 'docker.{0}.total'.format(mtype)
                    stats[mkey] = metric[1]
                elif 'Space Available' in metric[0]:
                    mkey = 'docker.{0}.free'.format(mtype)
                    stats[mkey] = metric[1]

                if mkey in old_aliases and mkey in stats:
                    stats[old_aliases[mkey]] = stats[mkey]
